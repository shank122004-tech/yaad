...
async function _generateQuizQuestions(exam, count, type) {
    ...
    _renderQuizQuestion(quiz, groupId, memberNames) {
      ...
      if (quiz.status === 'countdown') {
        body.innerHTML = `<div style="text-align:center;padding:40px 20px;">
          ...
          <div style="font-size:13px;color:rgba(26,26,38,0.70);">Battle starting in a moment…</div>
        </div>`;
        return;
      }
        ...
        <div class="cf-quiz-battle-wrap" style="animation:slideIn 0.3s ease;">
          ...
          <div style="margin-bottom:10px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px;">
              ...
              <span id="cf-gqtimer-label" style="font-size:11px;font-weight:800;color:rgba(26,26,38,0.70);">30s</span>
            </div>
    ...
    _renderQuizResults(quiz, memberNames) {
      ...
      body.innerHTML = `
        <div style="text-align:center;padding:16px 0 10px">
          ...
          ${winner ? `<div style="font-size:13px;color:rgba(26,26,38,0.7);margin-bottom:12px">🥇 Winner: <strong style="color:#f59e0b">${memberNames&&memberNames[winner[0]]?memberNames[winner[0]]:(winner[0]===myUid?'You':'Player')}</strong> · ${winner[1].xp} XP · ${winner[1].correct}/${totalQ} correct</div>` : ''}
        </div>
          ...
          ${sorted.map(([u,s],i) => {
            ...
            return `<div style="display:flex;align-items:center;gap:10px;background:${isMe?'rgba(108,99,255,0.12)':'rgba(255,255,255,0.03)'};border:1px solid ${isMe?'rgba(108,99,255,0.35)':'rgba(255,255,255,0.07)'};border-radius:10px;padding:9px 12px;">
              ...
              <div style="flex:1;min-width:0">
                ...
                <div style="font-size:11px;color:rgba(26,26,38,0.70);margin-top:2px">✅ ${s.correct} correct · ❌ ${s.wrong} wrong · ${acc}% acc</div>
              </div>
              <div style="text-align:right;flex-shrink:0">
                <div style="font-size:15px;font-weight:800;color:#f59e0b">⚡${s.xp}</div>
                <div style="font-size:10px;color:rgba(26,26,38,0.70)">${s.total}/${totalQ} ans</div>
              </div>
            </div>`;
          }).join('')}
        </div>
   ...
   * ───────────────────────────────────────────────────────────── */
  function init() {
    // CRITICAL FIX: Wait for Firebase to be ready before initializing CF
    if (!window._firebaseAuth || !window._firebaseDb || !window._firebaseFns) {
      console.warn('[CrackAI] Firebase not ready, retrying init in 100ms...');
      setTimeout(init, 100);
      return;
    }
  ...
  // Wait for Firebase SDK to initialize (it's loaded as a module in HTML)
  if (window.__firebaseReady) {
    // Firebase already initialized (early DOM ready)
    ...
    // Firebase not ready yet, wait for firebaseReady event
    console.info('[CrackAI] Waiting for Firebase initialization...');
    window.addEventListener('firebaseReady', () => {
      console.info('[CrackAI] Firebase ready! Initializing features...');
      if (document.readyState === 'loading') {
