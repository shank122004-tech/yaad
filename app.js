...
function updateRankPredictorDisplay(uid) {
  ...
  try {
    ...
    const avgAccuracy = Math.round(recentTests.reduce((sum, t) => sum + (t.accuracy || 0), 0) / recentTests.length);
    const avgScore = Math.round(recentTests.reduce((sum, t) => sum + (t.score || 0), 0) / recentTests.length);
    const estimatedRank = Math.round(1000 * (1 - avgAccuracy / 100));
    const percentile = avgAccuracy;
    const rankEl = modal.querySelector('[data-rank-predictor-rank]');
    if (rankEl) rankEl.textContent = estimatedRank.toLocaleString() || '---';
    ...
    if (scoreEl) scoreEl.textContent = avgAccuracy + '%';
  } catch(e) {
    console.error('[Rank Predictor] Error:', e);
  }
}

function hideAllScreensForMockTest() {
  const screenIds = ['examsScreen', 'classesScreen', 'allExamsScreen', 'competitiveExamsScreen', 'schoolExamsScreen', 'mainAppContainer', 'mainScreen', 'chatScreen', 'profileScreen'];
...
const MockTestInterfaceModule = (function() {
  ...
  const finishTest = () => { 
    ...
    document.getElementById('mockTestFullscreenModal').style.display = 'none'; 
