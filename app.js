...
const MockTestInterfaceModule = (function() {
  ...
  const openTest = async (examType) => {
    ...
    const dailyLimit = MockTestModule.checkDailyLimit(state.user, state.isPremium || false);
    ...
    try {
      ...
      if (!state.isPremium) { MockTestModule.incrementDailyTest(state.user); }
      testInProgress = true;
      displayQuestion();
      startTimer();
    } catch (error) {
  ...
  const finishTest = () => { 
    ...
    if (state && state.user && result.results) {
      const userXP = MockTestModule.getUserXP(state.user);
      ...
      MockTestModule.saveUserXP(state.user, userXP);
