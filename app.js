...
const MockTestInterfaceModule = (function() {
  ...
  const openTest = async (examType) => {
    ...
    const userID = state.user || state.uid || state.userId;
    ...
    try {
      ...
      loading.style.display = 'none';
      if (!result.success) { showError(result.error); modal.style.display = 'none'; showAllScreensAfterMockTest(); return; }
  ...
  const finishTest = () => { 
    ...
    if (state && state.user && result.results) {
      ...
      if (userID && userID !== 'undefined' && userID !== 'null') {
        ...
        console.log('[MockTest] XP saved - User:', userID, 'Total XP:', userXP.total);
      } else {
  ...
  const closeResults = () => { 
    ...
    // Show question section again if user retakes
    const questionSection = document.querySelector('.mock-question-section');
    if (questionSection) questionSection.style.display = '';
    const navSection = document.querySelector('.mock-nav-section');
    if (navSection) navSection.style.display = '';
