...
function hideAllScreensForMockTest() {
  try {
    ...
    // Hide any visible modal-like elements
    const modals = document.querySelectorAll('[class*="modal"]:not(#mockTestFullscreenModal):not(#mockTestResultsModal)');
...
const MockTestInterfaceModule = (function() {
  ...
  console.log('[MockTestInterface] Initializing...');

  // GET MODAL ELEMENTS
  function getModals() {
    return {
      fullscreen: document.getElementById('mockTestFullscreenModal'),
      results: document.getElementById('mockTestResultsModal'),
      loading: document.getElementById('mockLoadingOverlay'),
      dailyAlert: document.getElementById('mockDailyLimitAlert'),
      errorAlert: document.getElementById('mockErrorAlert'),
      questionSection: document.querySelector('.mock-question-section'),
      navSection: document.querySelector('.mock-nav-section'),
      headerLeft: document.querySelector('.mock-header-left')
    };
  }

  function validateModals() {
    const modals = getModals();
    if (!modals.fullscreen || !modals.results || !modals.loading) {
      console.error('[MockTest] Modal elements missing');
      return false;
    }
    return true;
  }
  ...
  function hideAllScreensForMockTest() {
    try {
      console.log('[MockTest] Hiding screens...');
      const screenIds = ['welcomeScreen', 'profileModal', 'authScreen'];
      ...
      console.error('[MockTest] Hide error:', e);
      return false;
    }
  }
  ...
  function showAllScreensAfterMockTest() {
    try {
      console.log('[MockTest] Restoring screens...');
      ...
      Object.keys(originalDisplayStates).forEach(key => delete originalDisplayStates[key]);
      
      document.body.style.overflow = '';
      
      const modals = getModals();
      if (modals.fullscreen) modals.fullscreen.style.display = 'none';
      if (modals.results) modals.results.style.display = 'none';
      if (modals.loading) modals.loading.style.display = 'none';
      
      return true;
    } catch(e) {
      console.error('[MockTest] Restore error:', e);
      return false;
    }
  }
  ...
  function displayQuestion() {
    try {
      ...
      if (!q) {
        console.error('[MockTest] No current question');
        return false;
      }

      const questionText = document.getElementById('mockQuestionText');
      if (!questionText) {
        console.error('[MockTest] Question element not found');
        return false;
      }
      ...
      document.getElementById('mockExamName').textContent = MockTestModule.getExamName(MockTestModule.getState().currentExam);

      for (let i = 0; i < 4; i++) {
        const optBtn = document.getElementById(`mockOpt${i}`);
        const optText = document.getElementById(`mockOptText${i}`);
        if (optBtn && optText) {
          optText.textContent = q.options[i] || '';
          optBtn.classList.remove('selected');
          if (MockTestModule.getState().userAnswers[q.index] === i) {
      ...
      if (nextBtn) nextBtn.disabled = q.index === q.total - 1;
      document.getElementById('mockNavCounter').textContent = `${q.index + 1} of ${q.total}`;

      questionsLoaded = true;
      return true;
    } catch(e) {
      console.error('[MockTest] Display question error:', e);
      return false;
    }
  }
  ...
  const openTest = async (examType) => {
    console.log('[MockTest] openTest:', examType);
    
    if (!examType || !MockTestModule || !MockTestModule.EXAM_TYPES[examType]) {
    ...
    if (!hideAllScreensForMockTest()) return false;
    ...
    modals.fullscreen.style.display = 'flex';
    modals.loading.style.display = 'flex';
    modals.results.style.display = 'none';
    modals.results.style.visibility = 'hidden';

    if (modals.questionSection) modals.questionSection.style.display = '';
    if (modals.navSection) modals.navSection.style.display = '';
    if (modals.headerLeft) modals.headerLeft.style.display = '';
    ...
    if (!dailyLimit.allowed && !state.isPremium) {
      ...
      modals.fullscreen.style.display = 'none';
      showAllScreensAfterMockTest();
      if (modals.dailyAlert) modals.dailyAlert.style.display = 'flex';
      return false;
    }

    try {
      console.log('[MockTest] Loading exam...');
      const result = await MockTestModule.startMockTest(examType, { questionCount: 10, shuffle: true });

      if (!result.success) {
        console.error('[MockTest] Failed:', result.error);
        modals.loading.style.display = 'none';
        if (modals.errorAlert) {
          document.getElementById('mockErrorMessage').textContent = result.error;
          modals.errorAlert.style.display = 'flex';
        }
        setTimeout(() => {
          modals.fullscreen.style.display = 'none';
          showAllScreensAfterMockTest();
      ...
      const testState = MockTestModule.getState();
      if (!testState.questions || testState.questions.length === 0) {
        console.error('[MockTest] No questions loaded');
        modals.loading.style.display = 'none';
        if (modals.errorAlert) {
          document.getElementById('mockErrorMessage').textContent = 'No questions available';
          modals.errorAlert.style.display = 'flex';
        }
        setTimeout(() => {
          modals.fullscreen.style.display = 'none';
      ...
      console.log('[MockTest] Questions loaded:', testState.questions.length);
      modals.loading.style.display = 'none';
      testInProgress = true;
      ...
      modals.loading.style.display = 'none';
      if (modals.errorAlert) {
        document.getElementById('mockErrorMessage').textContent = error.message;
        modals.errorAlert.style.display = 'flex';
      }
      setTimeout(() => {
        modals.fullscreen.style.display = 'none';
  ...
  const submitTest = () => {
    if (!testInProgress || !questionsLoaded) return;
    finishTest();
  };
  ...
  const finishTest = () => {
    ...
    if (state?.user && result.results) {
      const userID = state.user.uid || state.user.id;
      if (userID && window.processMockTestResults) {
        window.processMockTestResults(result.results);
      }
    }

    const modals = getModals();
    if (modals.questionSection) modals.questionSection.style.display = 'none';
    if (modals.navSection) modals.navSection.style.display = 'none';
    if (modals.headerLeft) modals.headerLeft.style.display = 'none';
  ...
  const displayResults = (results) => {
    ...
    modals.results.style.display = 'flex';
    modals.results.style.visibility = 'visible';
    modals.results.scrollTop = 0;
  };
...
// ═══════════════════════════════════════════════════════════════
// ANALYTICS & RANK PREDICTOR MODULE
// ═══════════════════════════════════════════════════════════════
