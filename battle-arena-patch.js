...
(function () {
  ...
  const POLL_ACTIVE_GAME = 500;             // ms — v4.0.0 FLICKER FIX: reduced to 500ms for 8x faster question display (was 1000ms)
  ...
  */
  ...
  window.BA = {
    ...
    _renderActiveQuiz(battle) {
      ...
      // AGGRESSIVE DEBOUNCE v4.0.0: Ultra-smart rendering to prevent all flicker
      const now = Date.now();
      ...
      // CRITICAL FIX v4.0.0: When question changes, render IMMEDIATELY with 0 debounce
      // If nothing changed and we rendered recently, skip full re-render
      if (existingQuiz && !hashChanged && !questionChanged && timeSinceRender < 400) {
      ...
      const quizHTML = `
        <div class="ba-active-wrap" style="will-change:auto;backface-visibility:hidden;">
          <div class="ba-quiz-header">
            ...
            <span class="ba-quiz-timer" data-battle-timer style="background:linear-gradient(135deg,#ef4444,#dc2626);margin-left:8px;display:inline-block;min-width:65px;text-align:center;border-radius:6px;padding:6px 8px;font-size:12px;font-weight:700;color:white;transition:none;">⏱️ 5:00</span>
      ...
      if (existingWrap) {
        ...
        existingWrap.style.transition = 'opacity 0.12s ease-out';
        ...
        setTimeout(() => {
          ...
          if (newWrap) {
            ...
            newWrap.style.transition = 'opacity 0.15s ease-in';
