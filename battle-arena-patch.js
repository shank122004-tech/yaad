...
(function () {
  ...
  const POLL_ACTIVE_GAME = 1000;             // ms — FIXED v3.2.7: increased to 1000ms to prevent refresh loop
    ...
    async _pollGameBattle(battleId) {
      ...
      try {
        ...
        if (data.status === 'waiting' && this._pollGameInterval) {
          ...
          clearInterval(this._pollGameInterval);
          // FIX v3.2.7: Reduced polling to 4000ms (every 4 seconds) to save Firebase reads
          // Questions cached, only polling for answers/progress
          this._pollGameInterval = setInterval(() => this._pollGameBattle(battleId), 4000);
        }
