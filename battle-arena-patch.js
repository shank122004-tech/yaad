  ...
  */
  ...
  window.BA = {
    ...
    async _pollGameBattle(battleId) {
      ...
      try {
        ...
        if (data.status === 'waiting' && this._pollGameInterval) {
          ...
          clearInterval(this._pollGameInterval);
          // FIX v3.2.8: Professional polling - 2000ms for 10-user battles to prevent lag
          // Questions cached, only polling for answers/progress
          ...
          if (creatorUid && creatorUid === myUid) {
            ...
            while (updateAttempts < maxAttempts && !updateSuccess) {
              try {
                ...
                battleState.countdownLocked = false;  // Unlock after successful activation
    ...
    async _renderBattleWinner(battle) {
      // CRITICAL FIX v3.2.8: Fetch fresh battle data to ensure XP is fully updated
      let freshBattle = battle;
      try {
        const db = window._firebaseDb;
        const { doc, getDoc } = window._firebaseFns;
        const freshSnap = await getDoc(doc(db, 'publicBattles', battle.id || this._activeBattleId));
        if (freshSnap && freshSnap.exists()) {
          freshBattle = freshSnap.data();
          freshBattle.id = freshSnap.id;
        }
      } catch (err) {
        console.warn('[Results] Could not fetch fresh data:', err.message);
