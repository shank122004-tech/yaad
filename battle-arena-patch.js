...
(function () {
  ...
  const POLL_ACTIVE_GAME = 2500;            // ms — PROFESSIONAL FIX v3.2.9: increased to 2500ms for ultra-smooth battles
  const POLL_WAITING_ROOM = 3000;           // ms — waiting room slower polling
  ...
  // CRITICAL FIX v3.2.9+: Realtime Database for battle data (faster, cheaper)
  const realtimeDb = window._firebaseRealtimeDb || null;
  
  // Helper: Get battle from Realtime Database
  async function getBattleRealtimeDb(battleId) {
    if (!realtimeDb) return null;
    try {
      const snapshot = await realtimeDb.ref(`battles/${battleId}`).once('value');
      return snapshot.exists() ? { id: battleId, ...snapshot.val() } : null;
    } catch (err) {
      console.warn('[RealDB] getBattle failed:', err.message);
      return null;
    }
  }
  
  // Helper: Update battle in Realtime Database
  async function updateBattleRealtimeDb(battleId, updates) {
    if (!realtimeDb) return false;
    try {
      await realtimeDb.ref(`battles/${battleId}`).update(updates);
      return true;
    } catch (err) {
      console.warn('[RealDB] updateBattle failed:', err.message);
      return false;
    }
  }
  ...
  */
  ...
  window.BA = {
    ...
    _renderActiveQuiz(battle) {
      ...
      // CRITICAL FIX v3.2.7: Defensive initialization - ensure all properties exist
      if (!battle) {
      ...
      // CRITICAL FIX v3.2.9: Skip full re-render if nothing changed and timer exists (extended 800ms)
      if (!hashChanged && !questionChanged && timeSinceRender < 800 && document.getElementById('ba-qtimer-fill')) {
    ...
    async _submitAnswer(battleId, qi, chosenIdx) {
      ...
      try {
        ...
        // 🎯 NEW v3.2.9: Write to Realtime Database (instant, no lag)
        const updates = {
          ['quiz/answers/user_' + myUid + '_q' + String(qi)]: 
        ...
        // Try Realtime DB first (instant), fallback to Firestore
        const realdbSuccess = await updateBattleRealtimeDb(battleId, updates);
        if (!realdbSuccess) {
          // Fallback to Firestore if RealDB fails
          const db = window._firebaseDb;
          const { doc, updateDoc } = window._firebaseFns;
          const fsUpdates = {
            ['quiz.answers.' + 'user_' + myUid + '_q' + String(qi)]: 
