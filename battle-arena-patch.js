  ...
  */
  ...
  window.BA = {
          ...
          if (creatorUid && creatorUid === myUid) {
            ...
            while (updateAttempts < maxAttempts && !updateSuccess) {
              try {
                ...
                if (success) {
                  updateSuccess = true;
                  console.info('[Countdown] ✅ Battle activated via RealDB (attempt ' + updateAttempts + ')');
            ...
            try {
              ...
              if (freshSnap.exists()) {
                ...
                if (completeData.questions && completeData.questions.length > 0) {
                  ...
                  console.info('[Battle] Questions cached after countdown:', completeData.questions.length);
                }
    ...
    _startQuestionTimer(battleId, qi, questionStartedAt) {
      ...
      const MIN_UPDATE_INTERVAL = 100;  // Only update DOM every 100ms max
      ...
      const tick = () => {
        ...
        // OPTIMIZATION: Only update DOM if value changed AND minimum interval passed
        if (remaining !== lastRenderedValue && (now - lastDomUpdateTime) >= MIN_UPDATE_INTERVAL) {
          ...
          if (label) {
            label.textContent = remaining + 's';
            if (remaining <= 5) label.style.color = '#ef4444';
            else if (remaining <= 10) label.style.color = '#f59e0b';
            else label.style.color = 'rgba(200,195,255,0.5)';
          }
        }
        ...
        if (remaining <= 0 && elapsedMs >= QUESTION_TIME * 1000) {
          ...
          // Use lower frequency requestAnimationFrame calls
