  ...
  window.BA = {
    ...
    _renderBattleRoom(battle) {
      ...
      if (battle.status === 'waiting') {
        ...
        // CRITICAL FIX: Check if we already have a waiting room and only update the player list instead of re-rendering everything
        ...
        if (existingWaitingRoom) {
          ...
          if (statusEl) {
            statusEl.innerHTML = `<div style="font-size:13px;color:#4ade80;font-weight:700;">🟢 Waiting for players...</div><div style="font-size:12px;color:rgba(26,26,38,0.70);margin-top:4px;">${playerCount}/${MAX_PLAYERS} joined · ${slotsLeft} slots left</div>`;
          }
          return;
        }
    ...
    _renderActiveQuiz(battle) {
      ...
      // CRITICAL FIX v3.2.11: Prevent flickering by checking if battle DOM already exists
      // If we're just updating minor things, don't replace the entire body
      const existingQuiz = body.querySelector('.ba-active-wrap');
      if (existingQuiz && !battle.questions) return; // Prevent flickering on incomplete data
      ...
      // AGGRESSIVE DEBOUNCE v3.2.9: Skip render if nothing important changed
      const now = Date.now();
      ...
      // CRITICAL FIX v3.2.11: Enhanced debounce to prevent flicker
      // Skip render if nothing changed for this question+user combo
      // Reduced debounce time from 1200ms to 400ms for faster transitions
      if (!hashChanged && !questionChanged && timeSinceRender < 400 && document.getElementById('ba-qtimer-fill')) {
        // Only lightweight DOM updates (no full re-render)
        try {
    ...
    async _submitAnswer(battleId, qi, chosenIdx) {
      ...
      if (this._activeBattleId) {
        ...
        }, 50);  // OPTIMIZED: reduced from 150ms to 50ms for instant question transitions
      }
    },
