/**
 * battle-arena-patch.js — CrackwithAI Online Battle Arena v3.3.0 [NO QUESTION TIMER]
 * ═══════════════════════════════════════════════════════════════════
 *  🎯 VERSION 3.3.0 CHANGES:
 *  ✅ REMOVED: 15-second per-question timer completely
 *  ✅ UNCHANGED: 5-minute battle timer (stays active)
 *  ✅ INSTANT: Next question appears when user answers (no delay)
 *  ✅ RESULTS: Show when 5-min timer expires OR all questions answered
 *  ✅ COINS: Earned in battle, updated in user profile instantly
 *  ✅ NO TIMER DISPLAY: "Take your time!" message instead
 *  ✅ POLLING FASTER: 1000ms instead of 1500ms for instant transitions
 * 
 *  FEATURES:
 ...
 *
 *  FIREBASE COST OPTIMIZATION v3.3.0:
 *  ✅ Questions cached in memory — fetched ONCE at battle start
 *  ✅ Polling reduced to 1000ms (every 1 second) during active battle for INSTANT transitions
 ...
 *  - Uses getDoc polling (1s active game, 1s list) NOT onSnapshot listeners
 ...
 *
 *  IMPROVEMENTS v3.3.0:
 *  ✅ REMOVED: 15-second countdown timer per question (was QUESTION_TIME = 15)
 *  ✅ REMOVED: Timer display from question interface
 *  ✅ REMOVED: Auto-skip functionality (no timer to trigger it)
 *  ✅ DISABLED: _startQuestionTimer now a no-op (returns immediately)
 *  ✅ ENHANCED: Polling interval 1000ms for instant next question display
 *  ✅ INTACT: 5-minute battle timer (300 seconds) — only timer that matters
 *  ✅ INTACT: Coins earning and display system (working perfectly)
 *  ✅ INTACT: Results page (shows when timer expires or all questions answered)
 * ═══════════════════════════════════════════════════════════════════
 */
...
(function () {
  ...
  const POLL_ACTIVE_GAME = 1000;            // ms — PROFESSIONAL FIX v3.2.9: reduced to 1000ms for instant question transitions (even faster)
  const POLL_WAITING_ROOM = 1000;           // ms — waiting room faster polling for real-time player updates (optimized from 3000ms)
  const QUESTION_TIME    = 0;             // DISABLED v3.3.0: No per-question timer - users must answer to proceed (only 5-min battle timer active)
  ...
  */
  ...
  const BattleTimer = {
    ...
    start(battleId, durationSeconds = 300) {
      ...
      // v3.3.0 NO-FLICKER FIX: Use requestAnimationFrame for smooth, flicker-free updates
      const updateTimer = () => {
        const now = Date.now();
        const remaining = Math.max(0, endTime - now);
        const minutes = Math.floor(remaining / 60000);
        
        // Only update DOM if minutes changed AND enough time has passed
        if (minutes !== timerState.lastDisplayedMinutes && (now - timerState.lastUpdateTime) > 900) {
          ...
          if (timerEl) {
            ...
            const newText = `⏱️ ${minutes}:00`;
            
            // Only update if text is actually different (prevent unnecessary reflow)
        ...
        if (remaining <= 0) {
          ...
          timerState.intervalId = setTimeout(updateTimer, 1000);
        }
      };
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      /* v3.3.0 NO-FLICKER FIX: Stable timer display */
      [data-battle-timer] {
        min-width: 65px; text-align: center; display: inline-block;
        font-variant-numeric: tabular-nums; /* Prevent width changes from text */
      }
      .ba-timer-warning { animation: none; }
      .ba-timer-critical { animation: none; }
  ...
  window.BA = {
    ...
    _renderActiveQuiz(battle) {
      ...
      // v3.3.0 NO-FLICKER FIX: Preserve timer content before re-render
      const existingTimerEl = document.querySelector('[data-battle-timer]');
      const savedTimerContent = existingTimerEl ? existingTimerEl.textContent : '⏱️ 5:00';
      const savedTimerClass = existingTimerEl ? existingTimerEl.className : 'ba-quiz-xp-pill';
      ...
      body.innerHTML = `
        <div class="ba-active-wrap" style="will-change:auto;backface-visibility:hidden;">
          <div class="ba-quiz-header">
            ...
            <span class="${savedTimerClass}" data-battle-timer style="background:linear-gradient(135deg,#ef4444,#dc2626);margin-left:8px;">${savedTimerContent}</span>
