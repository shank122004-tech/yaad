    ...
    _handleCountdown(data, battleId) {
      ...
      const tick = async () => {
        ...
        if (remaining > 0) {
          ...
          // FIXED v3.2.4: Reset flags IMMEDIATELY to prevent re-triggering
          this._countdownShown = false;
          this._countdownInProgress = false;
          
          // Activate battle in Firestore (creator only) with proper error handling
          const myUid = uid();
          ...
          if (creatorUid && creatorUid === myUid) {
            ...
            // CRITICAL FIX v3.2.4: Proper error handling + retry logic
            ...
            while (updateAttempts < maxAttempts && !updateSuccess) {
              try {
                ...
                // FIX: Validate questions exist and are array
                const questionsArray = Array.isArray(data.questions) ? data.questions : [];
                if (questionsArray.length === 0) {
                  console.warn('[Countdown] WARNING: No questions found in battle data');
                }
                
                // CRITICAL FIX: Split update into 2 operations to avoid Firestore document size limit
                // Operation 1: Update status + basic fields
                await updateDoc(doc(db, 'publicBattles', battleId), {
                  status: 'active',
                  startedAt: Date.now()
                });
                ...
                // Operation 2: Update quiz object separately
                await updateDoc(doc(db, 'publicBattles', battleId), {
                ...
                updateSuccess = true;
                console.info('[Countdown] ✅ Battle activated successfully (attempt ' + updateAttempts + ')');
                
              } catch(err) {
                console.error('[Countdown] ⚠️ Attempt ' + updateAttempts + ' failed:', err.message);
                
                // Specific error handling
                if (err.code === 'permission-denied') {
                  toast('❌ Permission denied - check Battle plan status', 4000);
                  updateAttempts = maxAttempts;
                } else if (err.code === 'resource-exhausted') {
                  toast('❌ Database quota exceeded - try again later', 4000);
                  updateAttempts = maxAttempts;
                } else if (err.code === 'invalid-argument') {
                  toast('❌ Invalid battle data - please recreate the battle', 4000);
                  updateAttempts = maxAttempts;
                } else {
                  // Transient errors - retry
                  if (updateAttempts < maxAttempts) {
                    console.info('[Countdown] Retrying in 200ms...');
            ...
            if (!updateSuccess) {
              console.error('[Countdown] ❌ Failed to activate battle after ' + maxAttempts + ' attempts');
              toast('❌ Battle failed to start. Please try again.', 4000);
              return;
            }
          }
          ...
          setTimeout(() => {
            ...
            // Force immediate poll to pick up 'active' status
            this._pollGameBattle(battleId);
