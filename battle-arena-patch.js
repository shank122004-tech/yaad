/**
 * battle-arena-patch.js — CrackwithAI Online Battle Arena v4.0.0 [NO TIMER - RESULTS ON COMPLETION]
 * ═══════════════════════════════════════════════════════════════════
  ...
  */
  ...
  window.BA = {
    ...
    _renderActiveQuiz(battle) {
      ...
      // CRITICAL FIX v4.1.0: MUCH stricter debounce to prevent flicker from rapid polls
      // Only do LIGHT UPDATE if: same question, same state, AND at least 800ms passed since last render
      if (existingQuiz && !hashChanged && !questionChanged && timeSinceRender < 800) {
      ...
      // LOCK RENDER SYSTEM v4.1.0: Prevent other polls from interfering
      ...
      if (existingWrap) {
        ...
        setTimeout(() => {
          ...
          if (newWrap) {
            ...
            // Release render lock after fade-in completes (~150ms)
            setTimeout(() => {
        ...
        body.innerHTML = quizHTML;
        // Release lock immediately for first load
        this._renderInProgress = false;
      }
